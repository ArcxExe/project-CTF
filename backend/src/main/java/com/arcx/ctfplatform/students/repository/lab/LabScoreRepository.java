...
public interface LabScoreRepository extends JpaRepository<LabScore, UUID> {
    ...
    @Query("SELECT MAX(ls.score) FROM LabScore ls JOIN ls.student s WHERE s.groupId = :groupId AND s.status = 'ACTIVE'")
    Optional<Integer> findMaxScoreByGroupId(@Param("groupId") UUID groupId);

    @Query("SELECT MAX(ls.score) FROM LabScore ls JOIN ls.student s JOIN AcademicGroup g ON s.groupId = g.id WHERE g.streamId = :streamId AND s.status = 'ACTIVE'")
    Optional<Integer> findMaxScoreByStreamId(@Param("streamId") UUID streamId);

    @Query("SELECT MAX(ls.score) FROM LabScore ls JOIN ls.student s WHERE s.status = 'ACTIVE'")
